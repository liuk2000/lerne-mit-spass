...
document.getElementById('repeat-btn').addEventListener('click', () => {
    ...
    document.querySelector('.main .container .main__container').innerHTML = `<div class="card mono">
        <div>
            <div class="german">
                <strong></strong>
            </div>
...
for (const subject_card of document.querySelectorAll('section.subjects div.subject.card')) {
    subject_card.addEventListener('click', () => {
        const name = subject_card.querySelector('div.subject-name input[type="hidden"]').value
        set_subject(name)
        init_page()
    })
}
...
document.getElementById('cancel-load-data-btn').addEventListener('click', () => {
    const subject = get_subject()
    if (subject) {
...
document.querySelector('section.menu span#subject').addEventListener('click', () => {
    display_subjects()
})
...
const load_data = () => {
    ...
    fetch(uri)
        ...
        .then(json => {
            data = json
            document.querySelector('.container > h4 > span.category').innerHTML = category
            current_index = 0
...
const init_page = () => {
    ...
    document.querySelector('section.menu span#subject img').setAttribute('src', `images/flag_${subject}.svg`)
    topics = get_topics()
    console.log('topics', topics)
    if (!topics) {
        display_topics()
...
window.addEventListener('load', () => {
    document.querySelector('.container > h4 > span.category-change').addEventListener('click', () => {
        display_topics()
    })
    init_page()
})